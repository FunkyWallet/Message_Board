...
<body style="text-align: center;">
	<?php
		...
		// Check for file/file data existence.
			if ((file_exists("messages.txt") === true) && (filesize("messages.txt") > 0)) {
				$MessageArray = file("messages.txt");
				$Count = count($MessageArray);
			// Iterate through each message and extract the subject.
				for ($i=0; $i < $Count; ++$i) {
					$CurrMsg = explode("~", $MessageArray[$i]);
					$ExistingSubjects[] = $CurrMsg[0];
				}
			} // End file-checking if()
		// Check new subject against the ExistingSubjects array
			if (in_array($Subject, $ExistingSubjects) === true) {
				echo "<p style='color:red;'>The subject you entered already exists!<br /> Please enter a new subject and try again.</p>" ;
				echo "<p style='color:red;'> Your message was not saved!</p>";
				$Subject = "";
			} else {
